...
const CandidatePipeline = () => {
    ...
    const runATSAnalysis = async () => {
        ...
        setTimeout(() => {
            const keywords = ['React', 'Node', 'Premium UI', 'Architecture'];
            const found = Math.random() > 0.4;
            setATSResult({
                status: found ? 'Shortlisted' : 'Not Shortlisted',
                score: Math.floor(Math.random() * 40) + 60,
                match: found ? 'Candidate shows strong expertise in modern architecture and frontend systems.' : 'Candidate lacks specific experience in enterprise-scale React state management.',
                recommendation: found ? 'Proceed to technical screening' : 'Focus on candidates with higher React proficiency'
            });
